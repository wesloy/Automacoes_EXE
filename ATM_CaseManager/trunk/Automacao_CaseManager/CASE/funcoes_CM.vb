...
Public Class funcoes_CM
    ...
    Public Function capturarAlertas() As Boolean
        ...
        Dim carregaVolumeCompletoDoDia As Boolean = False
        Dim permitirVolumeCompletoDoDia As Boolean = False
        Dim qtdMaximaVolumeCompletoDoDia As Integer = 3
        Dim contVolumeCompletoDoDias As Integer = 0
        Dim intervaloA() As String = {INTERVALO_IMPORTACAO_COMPLETA(0), INTERVALO_IMPORTACAO_COMPLETA(1), INTERVALO_IMPORTACAO_COMPLETA(2)}
        'Dim resultadoA() As String = {0, 0, 0}
        ...
        'calculando dinamicamente um intervalo para as importações com o
        'volume completo do dia na planilha exportada do case manager
        '01/10/2019 - Rafael
        'Durante um intervalo de 1hrs 3x ao dia seram feitas importações da planilha com o periodo completo do dia
        If ((Now.Hour >= intervaloA(0)) And (Now.Hour < (intervaloA(0) + 1))) _
            Or ((Now.Hour >= intervaloA(1)) And (Now.Hour < (intervaloA(1) + 1))) _
            Or ((Now.Hour >= intervaloA(2)) And (Now.Hour < (intervaloA(2) + 1))) Then
            'Rotina de importação do periodo completo do dia
            msk_intervalo_min.SetValue("00:00")
        ElseIf Now.Hour > 8 Then
            'importação somente de um periodo de tempo especifico
            msk_intervalo_min.SetValue(Format(Now.AddHours(INTERVALO_IMPORTACAO_HORAS * -1), "HH:mm").ToString)
        Else
            msk_intervalo_min.SetValue("00:00")
        End If
        'fim dos ajustes
