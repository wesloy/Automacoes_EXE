...
Public Class atmImportacaoCASE
    ...
    Public Function importarCase(diretorio As String, ctrl_status As Control, ctrl_cont_importados As Control, ctrl_cont_naoImportados As Control, ctrl_cont_total As Control, Optional ByVal CaseON As Boolean = True, Optional ByVal acaoContingencial As Boolean = False) As Boolean
        ...
        Dim tabelaATM As String = BD_ALERTA_DIGITAL.ToString
        ...
        Try
            ...
            'Carregando todos as maquinetas de politica
            contadorFOR = 0 'Zerando contador
            dt = Nothing
            sql = "select * from MX_sysMaquinetasBloqueadas where ativa = " & Objcon.valorSql(True) & ""
            dt = Objcon.RetornaDataTable(sql)
            Dim MaqBloqueadas(0 To dt.Rows.Count) As String
            If dt.Rows.Count > 0 Then 'Validando se tem volume
                For Each drRow As DataRow In dt.Rows 'Carregando Array
                    MaqBloqueadas(contadorFOR) = drRow("MAQUINETA")
                    contadorFOR = contadorFOR + 1
                Next
            End If
            ...
            'Carregando bins Especificos por fila
            Dim listagemBinsPorFila As New List(Of BinsPorFila)
            listagemBinsPorFila = BinsPorFilaEspecifica()
            ...
            While Not linha = 9
                ...
                'Higienização 7
                'Exclusão de registros por uma chave de exclusao, contendo: GRUPO-REGRA-CODIGO_EC
                'If importar Then
                '    Dim rx = Split(chave_exclusao, ";")
                '    For i = LBound(rx) To UBound(rx)
                '        If rx(i) = Replace(Trim(appExcel.Cells(linha, 7).Value) & "-" & Trim(appExcel.Cells(linha, 8).Value), " ", "") & "-" & Replace(appExcel.Cells(linha, 17).Value, " ", "") Then
                '            importar = False
                '            Exit For
                '        End If
                '    Next
                ...
                Dim codMaquineta As String
                If importar Then
                    codMaquineta = Replace(appExcel.Cells(linha, 17).Value, " ", "")
                    For i = LBound(MaqBloqueadas) To UBound(MaqBloqueadas)
                        If codMaquineta = MaqBloqueadas(i) Then
                            codFila = 94 'CM REPRESADO
                            acaoContingencial = False 'CASO SEJA UMA IMPORTAÇÃO CONTINGENCIAL IMPEDIR QUE SUBA PARA HUMANOS
                            status = 3 'SUBIR FINALIZADO, JÁ QUE NÃO TERÁ AÇÃO HUMANA E SERÁ APENAS PARA ESTUDOS
                            Exit For
                        End If
                    Next
                End If
                ...
                '+ um puxadinho....
                'Nova implementação em 24/04/2019 - Rafael
                'direcionar o BIN 376619 para a fila 97-CM CORPORATIVO
                If importar Then
                    If bin = "376619" Then
                        codFila = 97 'CM CORPORATIVO
                        status = 0 'Status para Humano
                    End If
                End If
                ...
                If importar Then
                    ...
                    If status = 5 And acaoContingencial = False And registrarDescarte = False Then
                        ...
                        With rsRobo
                            ...
                            'Campos novos em 18/11/2019
                            .Fields("CodFilaAtual").Value = Trim(appExcel.Cells(linha, 3).Value)
                            .Fields("CodFilaAntiga").Value = Trim(appExcel.Cells(linha, 5).Value)
        ...
        Catch ex As Exception
            Objcon.Desconectar()
            msgDeErros = "Erro: (" & Err.Number & ") " & ex.Message & " || " & Err.Description
    ...
    'retorna uma listagem com os bins permitidos para importar somente em uma determinada fila
    Public Function BinsPorFilaEspecifica() As List(Of BinsPorFila)
        Dim lista As New List(Of BinsPorFila)
        Try
            Dim BPF As BinsPorFila
            Dim tabela As DataTable
            Dim filaTemp As String = String.Empty
            Dim filaTemp2 As String = String.Empty
            sql = "Select filaEspecifica, BIN "
            sql += "From MX_B2K_BINsImportacao "
            sql += "Where 1 = 1 "
            sql += " And ativa = 0 "
            sql += "And filaEspecifica Is Not null "
            sql += "group by filaEspecifica, BIN "
            sql += "order by filaEspecifica asc "
            tabela = Objcon.RetornaDataTable(sql)
            If tabela.Rows.Count > 0 Then 'Validando se tem volume
                For Each drRow As DataRow In tabela.Rows 'Carregando Array
                    BPF = New BinsPorFila
                    BPF.fila = drRow("filaEspecifica")
                    BPF.bin = drRow("bin")
                    lista.Add(BPF)
                Next
            Else
    ...
    'valida se o bin especificado pode ser importado apenas para uma determinada fila
    Public Function PermiteImportarBinEspecificoParaFila(bin As String, fila As String, listaBinsPorFila As List(Of BinsPorFila)) As Boolean
        Try
            Dim retorno As Boolean = False
            If listaBinsPorFila.Count > 0 Then
                For Each item In listaBinsPorFila
                    If item.bin = bin AndAlso item.fila = fila Then
                        retorno = True
                        Exit For
                    Else
                        retorno = False
    ...
    'valida se o bin especificado deve ser importado somente para uma determinada fila
    Public Function ImportarBinDeFilaEspecifica(bin As String, listaBinsPorFila As List(Of BinsPorFila)) As Boolean
        Try
            Dim retorno As Boolean = False
            If listaBinsPorFila.Count > 0 Then
                For Each item In listaBinsPorFila
                    If item.bin = bin Then
                        retorno = True
                        Exit For
                    Else
                        retorno = False
